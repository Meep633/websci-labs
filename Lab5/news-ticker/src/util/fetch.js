...
async function getDB({path}) {
  try {
    let response = await axios.get("http://localhost:3000/db/" + path);
    return {
      success: true,
      output: response.data
    };
...
async function postDB({path, params}) {
  try {
    let response = await axios.post("http://localhost:3000/db/" + path, params);
    return {
      success: true,
      output: response.data
    };
...
async function putDB({path, params}) {
  try {
    let response = await axios.put("http://localhost:3000/db/" + path, params);
    return {
      success: true,
      output: response.data
    };
...
async function deleteDB({path}) {
  try {
    let response = await axios.delete("http://localhost:3000/db/" + path);
    return {
      success: true,
      output: response.data
    };
...
export {getCurrentNews, postLatestNews, postNews, putNews, deleteNews, getDB, postDB, putDB, deleteDB};